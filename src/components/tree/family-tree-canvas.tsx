 ...
 * @description Horizontal d3.tree canvas with children/_children collapse + zoom
 * @version 2.0.0
 * @updated 2026-07-19
 */
...
function pointX(d: HierarchyPoint): number {
  return d.y;
}

function pointY(d: HierarchyPoint): number {
  return d.x;
}
...
export function FamilyTreeCanvas({
  ...
  useEffect(() => {
    ...
    const treeLayout = d3
      .tree<HierarchyPersonNode>()
      .nodeSize([TREE_NODE_SIZE_Y, TREE_NODE_SIZE_X]);
    ...
    const nodeEnter = nodeBinding
      ...
      .attr(
        'transform',
        (d) =>
          `translate(${pointX(d.parent ?? d) - NODE_WIDTH / 2}, ${pointY(d.parent ?? d) - NODE_HEIGHT / 2})`
      )
      .style('opacity', 0)
      .on('click', (event, d) => {
    ...
    nodeEnter
      ...
      .attr(
        'transform',
        (d) =>
          `translate(${pointX(d) - NODE_WIDTH / 2}, ${pointY(d) - NODE_HEIGHT / 2})`
      )
      .style('opacity', 1);
    ...
    // Ensure collapse buttons exist after expand (for nodes that gained kids visibility)
    ...
    if (
      !didInitialZoomRef.current &&
      zoomBehaviorRef.current &&
      wrapperRef.current &&
      descendants.length > 0
    ) {
      didInitialZoomRef.current = true;
      const xs = descendants.map((d) => pointX(d));
      const ys = descendants.map((d) => pointY(d));
      ...
      svg
        ...
        .call(
          ...
          d3.zoomIdentity
            .translate(clientWidth / 2, wrapperRef.current.clientHeight / 2)
            .scale(scale)
            .translate(-midX, -midY)
        );
    }
  ...
  return (
    <div
      ref={wrapperRef}
      className="relative h-[85vh] w-full overflow-hidden rounded-xl border bg-muted/30 shadow-inner"
    >
      <svg
        ref={svgRef}
        className="h-full w-full cursor-grab outline-none active:cursor-grabbing"
      />
      <div className="pointer-events-none absolute bottom-4 left-4 select-none text-xs font-medium text-muted-foreground opacity-50">
