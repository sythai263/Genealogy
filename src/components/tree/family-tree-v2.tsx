...
const TreeNode = React.memo(
  ({ node, onSelect, onToggleCollapse, isSelected }: TreeNodeProps) => {
    ...
    const genderColor =
      person.gender === 1 ? 'border-blue-400' : 'border-pink-400';
...
const Minimap = React.memo(
  ({
    ...
    return (
      <div
        className='absolute bottom-4 right-4 bg-background/90 border rounded-lg p-2 shadow-lg z-50'
          ...
          <g transform={`scale(${scale})`}>
            {nodes.map(node => (
              <circle
                ...
                r={4 / scale}
                className={
                  node.person.gender === 1 ? 'fill-blue-400' : 'fill-pink-400'
                }
              />
            ))}
            <rect
...
export function FamilyTreeV2() {
  ...
  if (data && !autoCollapseApplied && data.people.length > 50) {
    ...
    for (const person of data.people) {
      if (
        person.generation >= collapseFromGen &&
        ...
        {({
          ...
          <>
            ...
            <div
              ...
              className='border rounded-lg bg-muted/30 overflow-hidden relative cursor-grab active:cursor-grabbing'>
              <TransformComponent
                wrapperStyle={{ width: '100%', height: '60vh' }}>
                <svg
                  width={layout.width}
